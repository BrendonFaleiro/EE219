function [A,Y,numIter,tElapsed,finalResidual]=reg_wnmfrule(X,W,k,lambda,option)
...
for i=1:option.iter
    ...
    if mod(i,10)==0 || i==option.iter
        ...
        XfitThis=A*Y;
        fitRes=norm(W.*(XfitPrevious-XfitThis));
        XfitPrevious=XfitThis;
        curRes=norm(W.*(X-XfitThis),'fro');
        if option.tof>=fitRes || option.residual>=curRes || i==option.iter
            fprintf('# of iterations is %0.0d. \n The final residual is %0.4f.\n',i,curRes);
            numIter=i;
